...
# Lithe Multi-installer
# a one line clone-and-compile for lithe:
#
#     ` $ curl -sL "https://raw.githubusercontent.com/lithe-project/lithe/master/scripts/multi_installer.sh" | bash
#
...
_set_wd() {
    if [ -d "$PWD/.git" ] && [ -f "$PWD/Makefile" ]; then
        ...
        if [ -d "$PWD"/lithe ]; then
            read -r -p "${1:-lithe directory already exists. Overwrite? [y/N]} " response
            case "$response" in
                [yY][eE][sS|[yY])
                    _colorize red "Overwriting old lithe directory" && echo
                    rm -rf "$PWD"/lithe
                    ;;
                *)
                    _fail "lithe directory already exists. Aborting..."
                    ;;
            esac
        fi
        mkdir lithe
        git clone -b master -q https://github.com/lithe-project/lithe lithe   >>build.log 2>&1 || _fail "Unable to clone git repository. Please see build.log for more information"
        cd lithe
    fi
}

_build_lithe() {
    _note "Building lithe from source (this might take a while)..."
    if [ -d build ]; then
...
_configure_linux() {
    if [ "$(awk -F= '/^NAME/{print $2}' /etc/os-release)" = "\"Ubuntu\"" ]; then
        ...
        _fail "Your OS version isn't supported by this installer. Please consider adding support for your OS to the project ('https://github.com/lithe-project')"
    fi
}
...
_configure_os() {
    ...
    case "${_unameOut}" in
        ...
        Darwin*)
            ...
            ;;
        *)
            _fail "This installer only runs on OSX 10.10+ and Ubuntu 16.04+. Please consider adding support for your OS to the project ('https://github.com/lithe-project')"
            ;;
    esac
    _note "Operating system configuration completed. You're halfway there!"
}

_note "Lithe Multi_Installer v1.0"
_colorize magenta "LITHE"
...
_note "Look in 'lithe/build/src/' for the executible binaries. See 'https://github.com/lithe-project/lithe' for more project support. Cowabunga!"
